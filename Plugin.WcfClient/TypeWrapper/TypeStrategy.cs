	...
	internal class TypeStrategy
	{
		...
		public String GetDefaultValue()
		{
			if(this.enumChoices != null)
				...
				default:
					if(this.IsContainer())
						return "length=0";
					else if(this.typeProperty.IsKeyValuePair || this.typeProperty.IsStruct)
						return this.TypeName;
					else return TypeStrategy.NullRepresentation;
				}
		}

		public Int32 GetEnumMemberCount()
			=> this.enumChoices != null ? this.enumChoices.Length : 0;
			...
			} else
			{
				...
				default:
					if(value.Equals(TypeStrategy.NullRepresentation))
						return null;
					else if(this.typeProperty.IsArray)
					{
						...
						return array;
					} else if(this.typeProperty.IsCollection)
					{
		...
		public String GetStringRepresentation(Object obj)
		{
			if(obj == null)
				...
				return obj.GetType().Equals(typeof(Boolean)) || this.enumChoices != null
					? obj.ToString()
					: String.Empty;
		...
		public Boolean HasMembers()
			=> this.typeProperty.IsComposite || this.typeProperty.IsNullable || this.typeProperty.IsKeyValuePair;

		public Boolean IsContainer()
			=> this.typeProperty.IsArray || this.typeProperty.IsDictionary || this.typeProperty.IsCollection;

		public Boolean IsDictionary()
			=> this.typeProperty.IsDictionary;

		public Boolean IsEnum()
			=> this.enumChoices != null;

		public Boolean IsKeyValuePair()
			=> this.typeProperty.IsKeyValuePair;

		public Boolean IsStruct()
			=> this.typeProperty.IsStruct;

		public void MarkAsInvalid()
			=> this.IsValid = false;

		private static Type GetClientType(String typeName)
			=> ClientSettings.GetType(typeName);

		private static Boolean TryParseXmlQualifiedName(String stringRepresentation, out XmlQualifiedName result)
		{
			Int32 num = stringRepresentation.LastIndexOf(":", StringComparison.Ordinal);
			if(num == -1)
			{
				result = new XmlQualifiedName(stringRepresentation);
				return true;
			}
			String ns = stringRepresentation.Substring(0, num);
			String text = stringRepresentation.Substring(num + 1);
			if(String.IsNullOrEmpty(text))
			{
				result = null;
				return false;
			}
			result = new XmlQualifiedName(text, ns);
			return true;
		}
	}
}